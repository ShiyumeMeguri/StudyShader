...
half4 GF2Fragment(v2f input) : SV_Target
{
    ...
    #else
    {
        ...
        half4 roughnessWeights = rmoTex.x * half4(-1.0, -0.0275, -0.572, 0.022) + half4(1.0, 0.0425, 1.04, -0.04);
        // Rim
        half rimIntensityBase = min(roughnessWeights.x * roughnessWeights.x, exp2(reflectIntensity * -9.28));
        rimIntensityBase = rimIntensityBase * roughnessWeights.x + roughnessWeights.y; //边缘光滑度
        half rimIntensity1 = rimIntensityBase * 1.04 + roughnessWeights.w;
        half rimIntensity2 = rimIntensityBase * -1.04 + roughnessWeights.z;
        rimFactor = GetRimRamp(normalDir, rimIntensity1, useRampMap);
        finalRimColor = metallicColor * rimIntensity2 + rimFactor;
